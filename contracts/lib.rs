...
mod erc20 {
    ...
    impl Erc20 {
        ...
        pub fn try_recover(&self) -> Result<()> {
            let vec: ink_prelude::vec::Vec<u8> = ink_prelude::vec::Vec::new();
            let sig = [0u8; 65];
            let mut output = <ink_env::hash::Blake2x256 as ink_env::hash::HashOutput>::Type::default();
            ink_env::hash_bytes::<ink_env::hash::Blake2x256>(&vec, &mut output);
            let _ = Self::env().ecdsa_recover(&sig, &output);
            Ok(())
        }
    }
